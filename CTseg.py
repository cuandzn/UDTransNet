...
# 组合好的图片重新导成CT，三维重建
ds = r"/root/data/rib dataset/51-60/0select"    # data s
ms = rs = r"/root/data/UDtransnet_dataset/51-60/0select"   # result s
...
for i in range(0, len(orderlist)):    # 遍历CT文件夹
    ...
    maskpath = os.path.join(ms + str('/') + orderlist[i],"udmask")
    ...
    outdcmpath = os.path.join(os.path.join(rs,orderlist[i]),"udoutdcm")
    ...
    for j in range(len(dcmlist)):   # 遍历CT
        if os.path.exists(os.path.join(dcmpath, dcmlist[j])):
            ...
            print("dcm not exist")
        mask = cv2.imread(os.path.join(maskpath,str(len(dcmlist) - j - 1) + ".bmp"))
